...
(* let maybePath (prefixParser: Parser<('a), UserState>) (finalParser: Parser<('b), UserState>) : Parser<'a list * 'b> =

    let pathParser =
        (prefixParser .>> pchar '.') .>>. many (attempt (prefixParser .>> pchar '.'))
        |>> (fun (a, b) -> List.append [ a ] b)


    (attempt (pipe2 pathParser finalParser (fun path parserResult -> path, parserResult)))
    <|> (finalParser |>> (fun (parserResult) -> [], parserResult))
 *)
...
let sepByNewLines p =
    choice [
        attempt (many1 (newline >>. preturn None <|> (spaces >>. p |>> Some)) |>> List.choose id)
        spaces >>. p |>>(fun x -> [x]) // Allow for one liners if just one construct
    ]
...
(* let (compositeLiteral: Parser<TypeNameWithGenericsInstantiationData -> Ast.CompositeLiteralData>) =

    (pchar '{' >>. sepBy expressionParser (pchar ',' >.< spaces) .>> pchar '}')
    |>> (fun result -> fun typeIdentifier -> UnresolvedStructReference(typeIdentifier), result) *)
