...
// TODO: Make it clear in the field names and such that 
// types have not been verified. For instance, I had to change
// `CallableDeclaration` in the function data to `Reference`
// since the first one made it sound like it was *for sure* a
...
(* and MethodAccessorData<'T> = {
    Name : MethodName
    CallData : AccessorCallData<'T> option
} *)
...
// Here bro
(* and AccessorCallDataExprArgs = {
    GenericsInstantiation : GenericsInstantiationData option
    FunctionArguments : ExpressionCallArguments
}

and AccessorCallDataPipeArgs = {
    GenericsInstantiation : GenericsInstantiationData option
    FunctionArguments : PipeCallArguments
}
 *)
...
(* and AccessorBaseType = 
    | InterfaceType of TypeNameWithGenericsInstantiationData
    | StructType of TypeNameWithGenericsInstantiationData
    | WrapperType of TypeNameWithGenericsInstantiationData *)

(* and AccessorBase = 
    | Variable of ScopedVariableData
    | Expr of Expression *)
